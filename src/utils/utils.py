...
def plot_finish(contours, spectrum=None, contour_labels=True, autofit=True):
    ...
    if autofit:
        ...
        # - old: contours.collections -> collection.get_paths()
        # - new: contours.get_paths()
        ...
        # Only autofit if we actually found any vertices
        if vertices:
    ...
    if contour_labels:
        pyplot.clabel(contours, inline=1, fmt="%.0e")
