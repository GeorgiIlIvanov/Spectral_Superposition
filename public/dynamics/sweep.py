"""
Grid sweep with multi-worker GPU parallelization.
Optimized for 8x L4 GPUs using work-stealing queue pattern.
Bulletproof: survives SSH disconnection, auto-resume, progress persistence.
"""
import numpy as np
import argparse
import os
...
from training_loop import (
    train_model, N_FEATURES, M_VALUES, S_VALUES, SEEDS,
...
def write_progress_state(results_dir: Path, state: dict):
    ...
    try:
        ...
        pass  # Non-critical, don't fail on state write errors
...
def get_output_filename(exp: dict) -> str:
    ...
    return f"n{exp['n_features']}_m{exp['m_hidden']}_s{exp['sparsity']:.6f}_seed{exp['seed']}.h5"
...
def worker_process(
    ...
    Multiple workers share each GPU to maximize utilization.
    """
    ...
    # Warm up torch.compile cache on first run
    warmup_done = False

    while not stop_flag.value:
        try:
            # Non-blocking get with timeout for graceful shutdown
        ...
        try:
            ...
            # First experiment warms up torch.compile
            use_compile = True
            if not warmup_done:
                # Smaller warmup run to compile kernels
                _ = train_model(
                    n_features=N_FEATURES,
                    m_hidden=64,
                    sparsity=0.5,
            ...
            # Progress logging (sparse to avoid clutter)
            if done % 50 == 0 or done == total_experiments:
                print(f"[{worker_name}] {done}/{total_experiments} "
                      f"({100*done/total_experiments:.1f}%) - "
                      f"m={exp['m_hidden']}, s={exp['sparsity']:.3f} in {elapsed:.1f}s")
    ...
    def update():
        while not stop_flag.value:
            try:
                ...
                elapsed = time.time() - start_time
                rate = completed / max(1, elapsed) * 3600  # experiments per hour
...
def run_sweep(
    ...
    """
    Run the full experiment sweep with multi-worker parallelization.

    Args:
        num_gpus: Number of GPUs to use
        workers_per_gpu: Number of worker processes per GPU
        results_dir: Directory for output HDF5 files
    ...
    if free_gb < 50:
        print(f"WARNING: Low disk space! Need ~80GB for full sweep.")
    ...
    print(f"=" * 60)
    print(f"Spectral Superposition Sweep v2 - BULLETPROOF MODE")
    print(f"=" * 60)
    ...
    # Shuffle experiments for better load balancing
    # (mix of different m_hidden sizes across workers)
    np.random.seed(42)
    ...
    for worker_id in range(total_workers):
        ...
        p = Process(
            ...
            args=(worker_id, gpu_id, task_queue, results_dir,
                  completed_counter, total_experiments, stop_flag)
        )
        p.start()
        processes.append(p)
...
def count_pending(results_dir: str = 'results_v2'):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run superposition experiment sweep')
