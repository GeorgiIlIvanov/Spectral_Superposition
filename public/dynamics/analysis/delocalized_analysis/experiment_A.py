...
def _process_file_A(args):
    ...
    try:
        ...
        return run_experiment_A_single_file(data, config)
...
def run_experiment_A(
    ...
    config_dict = {
        ...
        'eigengap_percentile': config.eigengap_percentile,
        'use_percentile_gap': config.use_percentile_gap,
    }

    # Process files in parallel
    all_results = []
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(_process_file_A, (fp, config_dict)): fp for fp in files}
        completed = 0
