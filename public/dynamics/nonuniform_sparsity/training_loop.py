"""
Training loop for non-uniform sparsity experiment.
Each feature i has sparsity S_i = i/n, creating a linear gradient from 0 to 1.
...
CHECKPOINT_STEPS = (
    ...
    list(range(7200, 25001, 1000))      # 7200, 8200, ..., 25000 (19 checkpoints)
)
...
def generate_nonuniform_sparse_batch(
    batch_size: int,
    n_features: int,
    ...
    """
    Generate batch with non-uniform sparsity per feature.

    Each feature i is zeroed out with probability sparsity_per_feature[i].
    S_i = i/n means feature 0 is never sparse (S=0) and feature n-1 is almost
    always sparse (S≈1).
    """
    ...
    # sparsity_per_feature shape: (n_features,) -> broadcast to (batch_size, n_features)
...
def train_model(
    ...
    """
    Train a superposition model with non-uniform sparsity and save checkpoints.

    Sparsity is linear: S_i = i/n for feature i.
    ...
    # Pre-compute sparsity per feature: S_i = i/n
    # Feature 0: S=0 (never masked), Feature n-1: S=(n-1)/n ≈ 1 (almost always masked)
    sparsity_per_feature = torch.arange(n_features, dtype=torch.float32, device=device) / n_features
    ...
    for step in range(total_steps + 1):
        ...
        if step in checkpoint_set:
            with torch.no_grad():
                ...
                x_eval = generate_nonuniform_sparse_batch(
        ...
        # Training step
        x = generate_nonuniform_sparse_batch(
    ...
    if output_path is not None:
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            f.attrs['sparsity_type'] = 'linear_nonuniform'  # S_i = i/n

            # Store the sparsity schedule for reference
            f.create_dataset('sparsity_per_feature',
                           data=np.arange(n_features, dtype=np.float32) / n_features)
...
if __name__ == '__main__':
    ...
    if args.test:
        print("Running test training with non-uniform sparsity...")
        ...
        # Verify sparsity gradient
        print(f"\nSparsity gradient: S_0=0, S_511={511/1024:.3f}, S_1023={1023/1024:.3f}")
        print("Test passed!")
