"""
Sweep for binned sparsity experiment.
Fixed m=256, n=1024, with S ∈ {0.1, 0.2, ..., 0.9} discrete bins.
...
def get_output_filename(exp: dict) -> str:
    ...
    return f"n{exp['n_features']}_m{exp['m_hidden']}_binned_seed{exp['seed']}.h5"
...
def run_sweep(
    ...
    """Run the binned sparsity experiment sweep."""
    ...
    print(f"=" * 60)
    print(f"Binned Sparsity Sweep (S ∈ {'{'}0.1, 0.2, ..., 0.9{'}'})")
    print(f"=" * 60)
    ...
    print(f"  Features per bin: ~{N_FEATURES // len(SPARSITY_VALUES)}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run binned sparsity experiment sweep')
